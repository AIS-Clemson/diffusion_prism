...
def load_img(image, path, mask, noise_std):
    ...
    # mask_out = copy.deepcopy(cv2.cvtColor(image, cv2.COLOR_RGB2BGR))
    ...
    mask_org = copy.deepcopy(cv2.cvtColor(mask, cv2.COLOR_RGB2BGR))


    # Add noise
    noise = np.random.normal(0, noise_std, mask.shape)
    # mask = mask/255.0 + noise# + n/255.0
    # mask = img_uint8(mask)
    
    image = image/255.0 + mask/255.0 + noise# + n/255.0
    # plt.imshow(img_uint8(temp))
    image = image*255
    ...
    return (2.*image - 1.), mask_out, mask_org, [R,G,B]




# path = opt.init_img
def load_img_org(image, path, mask, noise_std):
...
if __name__ == "__main__":
    ...
    if batch_generation:
        ...
        # img_path = './test_data/perlin_noise/'
                    ...
                    while i < num_sample:
                        try:
                            ...
                            for strength in np.arange(0.1, 0.9, 0.1):
                                ...
                                opt.strength = strength #random.uniform(0.4, 0.7)
                                ...
                                init_image, mask_out, mask_org, [R,G,B] = load_img(image, opt.init_img, mask, noise_std)
                                ...
                                image.save(os.path.join(sample_path, f"{image_name}_{i}.jpg"))
    
                                cv2.imwrite(os.path.join(sample_path, f"{image_name}_{i}_mask_org.png"), mask_org)
                                    ...
                                    for prompts in tqdm(data, desc="data"):
                                        ...
                                        if not opt.skip_save:
                                            ...
                                            for x_sample in x_samples:
                                                ...
                                                image_save_name = f"{image_name}_{i}_{x_sample_i}_{strength}.jpg"
