...
# path = opt.init_img
def load_img(image, path, noise_std):
    ...
    image = np.array(image)
    image = image + np.random.normal(0, noise_std, size=(w,h,3))
...
if __name__ == "__main__":
    ...
    if batch_generation:
        ...
        # img_path = 'D:/Data/D_Fire/images/'
        # img_path = './test_data/valid_images_all/valid_images_1/'
        img_path = './train/samples_mask/'
        ...
        style_path = './train/dendrite_sample/'
        ...
        style_file = style_path + random.choice(style_list)
        ...
        with torch.no_grad():
            with precision_scope("cuda"):
                with model.ema_scope():
                    ...
                    while i < num_sample:
                        try:
                            ...
                            # mask_file = './wildfire_images/38_mask.jpg'
                            ...
                            # opt.init_img = "./test_img/image_skeleton_dia.png"
                            # prompt_1 = "wildfire in forest, photo realistic, high resolution, 4k, HD"
                            # prompt_2 = "wildfire in the snow, clear, photo realistic, high resolution, 4k, HD"
                            # prompt_3 = 'fire burning in the smoke'
                            # prompt_4 = "wildfire in the forest, flame and fire in the smoke and snow, high resolution, photo realistic"
                            # prompt_5 = "wildfire in the desert, high resolution, photo realistic"
                            # prompt_6 = "wildfire in the city, high resolution, photo realistic"
                            prompt_6 = "a dendrite sample, high resolution, photo realistic"

                                               
                            # prompt_list = [prompt_1, prompt_2, prompt_3, prompt_4, prompt_5, prompt_6]
                            prompt_list = [prompt_6]
                            ...
                            # for strength in np.arange(0.1, 1.0, 0.1):
                                # for noise_std in [0, 0.01,0.05,0.1,0.2,0.5]:
                            ...
                            init_image = load_img(image, opt.init_img, noise_std)
                            init_image = init_image.to(device)
                            init_image = repeat(init_image, '1 ... -> b ...', b=batch_size)
                            init_latent = model.get_first_stage_encoding(model.encode_first_stage(init_image))  # move to latent space
                        
                            cv2.imwrite(os.path.join(sample_path, f"{image_name}_{base_count:04}_image.png"), cv2.cvtColor(np.array(image), cv2.COLOR_BGR2RGB))

                            
                            # Calculate variance and standard deviation
                            init_mean = torch.mean(init_latent)
                            init_var = torch.var(init_latent)
                            init_std = torch.std(init_latent)
                            
                            
                            # init_style = load_img(style, opt.init_img, noise_std)
                            
                            # To tensor
                            init_style = np.array(style).astype(np.float32) / 255.0
                            init_style = init_style[None].transpose(0, 3, 1, 2)
                            init_style = torch.from_numpy(init_style)
                            init_style = (2.*init_style - 1.)

                            init_style = init_style.to(device)
                            init_style = repeat(init_style, '1 ... -> b ...', b=batch_size)
                            init_style_latent = model.get_first_stage_encoding(model.encode_first_stage(init_style))  # move to latent space
                        
                            # cv2.imwrite(os.path.join(sample_path, f"{image_name}_{base_count:04}_style.png"), cv2.cvtColor(np.array(style), cv2.COLOR_BGR2RGB))

                            # Calculate variance and standard deviation
                            style_mean = torch.mean(init_style_latent)
                            style_var = torch.var(init_style_latent)
                            style_std = torch.std(init_style_latent)
                                
                            
                            new_latent = style_std * ((init_latent - init_mean) / init_std) + style_mean
                            
                            new_latent_image = latent2img(new_latent)
                            new_latent_image = cv2.resize(cv2.cvtColor(new_latent_image, cv2.COLOR_BGR2RGB), (512,512))
                            # plt.imshow(init_latent_image)
                            cv2.imwrite(os.path.join(sample_path, f"{image_name}_{base_count:04}_new_latent_image.png"), new_latent_image)
                            ...
                            init_style_image = latent2img(init_style_latent)
                            init_style_image = cv2.resize(cv2.cvtColor(init_style_image, cv2.COLOR_BGR2RGB), (512,512))
                            # plt.imshow(init_latent_image)
                            cv2.imwrite(os.path.join(sample_path, f"{image_name}_{base_count:04}_init_style_latent.png"), init_style_image)





                            image_save_name = f"{image_name}_{i}.png"
                            style_save_name = f"{image_name}_{i}_mask.png"
                            # cv2.imwrite(os.path.join(sample_path, mask_save_name), mask_out)
                
                            x_sample = model.decode_first_stage(new_latent)[0]
                            x_sample = torch.clamp((x_sample + 1.0) / 2.0, min=0.0, max=1.0)
                            x_sample = 255. * rearrange(x_sample.cpu().numpy(), 'c h w -> h w c')
                            image = Image.fromarray(x_sample.astype(np.uint8))
                            # plt.imshow(image)
                            cv2.imwrite(os.path.join(sample_path, f"{image_name}_{base_count:04}_dec_image.png"), cv2.cvtColor(np.array(image), cv2.COLOR_BGR2RGB))

                            # samples_image = latent2img(samples)
                            # samples_image = cv2.resize(cv2.cvtColor(samples_image, cv2.COLOR_BGR2RGB), (512,512))
                            # plt.imshow(image)
                            ...
                            for n in trange(opt.n_iter, desc="Sampling"):
                                ...
                                for prompts in tqdm(data, desc="data"):
                                    ...
                                    z_enc = sampler.stochastic_encode(new_latent, torch.tensor([t_enc]*batch_size).to(device))
                                    
                                        
                                    z_enc_image = latent2img(z_enc)
                                    z_enc_image = cv2.resize(cv2.cvtColor(z_enc_image, cv2.COLOR_BGR2RGB), (512,512))
                                    # plt.imshow(z_enc_image)
                                    # cv2.imwrite(os.path.join(sample_path, f"{image_name}_{base_count:04}_init_latent_image.png"), init_latent_image)
                            ...
                            #         # cache = get_local.cache # ->  {'your_attention_function': [attention_map]}
                                    ...
                                    if not opt.skip_save:
                                        ...
                                        for x_sample in x_samples:
                                            ...
                                            image_save_name = f"{image_name}_{i}_{x_sample_i}{n}.png"
                ...
                # np.save('dataset.npy',image_info)
