                ...
                with model.ema_scope():
                    ...
                    while i < num_sample:
                        try:
                            ...
                            opt.prompt = 'rainy day, urban, citizens, HD, high resolution, bus'
                            ...
                            latent = init_latent[0].detach().permute(1,2,0).cpu().numpy()
  
                            [h,w] = latent.shape[:2]
                            latent_1D = latent.reshape((h*w,4))
                                
                            
                            dimension = 3
                            n_components = dimension
                            estimator = PCA(n_components)
                            pca = estimator.fit_transform(latent_1D)
                            # pca.shape
                            pca_show = pca.reshape((h,w,dimension))
                            plt.imshow(pca_show); plt.axis('off')
                            
                            pca_show = (pca_show - np.min(pca_show))/(np.max(pca_show)-np.min(pca_show)) *255
                            cv2.imwrite('./tests/image_pca.png',pca_show)
                            ...
                            for n in trange(opt.n_iter, desc="Sampling"):
                                ...
                                for prompts in tqdm(data, desc="data"):
                                    ...
                                    latent = z_enc[0].detach().permute(1,2,0).cpu().numpy()
                                    [h,w] = latent.shape[:2]
                                    latent_1D = latent.reshape((h*w,4))
                                    ...
                                    pca_show = (pca_show - np.min(pca_show))/(np.max(pca_show)-np.min(pca_show)) *255
                                    cv2.imwrite('./tests/image_pca_after.png',pca_show)
                                    ...
                                    latent = samples[0].detach().permute(1,2,0).cpu().numpy()
                                    [h,w] = latent.shape[:2]
                                    latent_1D = latent.reshape((h*w,4))
                                    ...
                                    diffusion_after = x_samples[0].detach().cpu().permute(1,2,0).numpy()
                                    diffusion_after = (diffusion_after - np.min(diffusion_after))/(np.max(diffusion_after)-np.min(diffusion_after)) *255
                                    diffusion_after = cv2.cvtColor(diffusion_after, cv2.COLOR_BGR2RGB)

                                    plt.figure()
                                    plt.imshow(diffusion_after)
                                    cv2.imwrite('./tests/diffusion_after.png',diffusion_after)
