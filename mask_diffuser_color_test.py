...
def load_img(image_name, path, noise_std):
    ...
    mask_org = copy.deepcopy(image)

    for i in range(3):
        image[:, :, i] = image[:, :, i] * np.random.uniform(0, 1)
    ...
    return (2.*image - 1.), mask_out, mask_org
...
# def latent_fusion(z_enc_high, z_enc_low, mask):
#     size = z_enc_low.shape[2:]
#     mask = cv2.resize(mask, size)
    
#     for layer in z_enc_high[]
#     z_fusion = 
...
def plot3d(img, elevation=80, azimuth=-30):
    ...
    # If the image is RGB, convert it to grayscale for simplicity
    if len(img.shape) == 3:
        Z = np.mean(img, axis=2)  # Average the RGB channels
    else:
        Z = img
                ...
                with model.ema_scope():
                    ...
                    while i < num_sample:
                        try:
                            ...
                            # opt.prompt = "a dendrite sample in various background"
                            opt.prompt = "a dendrite sample with rich background"
    
    
                            # for strength in [0.1,0.2,0.5,0.7,0.9]:
                        
                            opt.scale = 50 #random.randint(5, 100)
                            opt.strength = random.uniform(0.1, 0.5)
                            ...
                            init_image, mask, mask_org = load_img(image_name, opt.init_img, noise_std)
                            init_image = init_image.to(device)
                            init_image = repeat(init_image, '1 ... -> b ...', b=batch_size)
                            ###############
                            init_rand = torch.zeros_like(init_image)
                            init_rand_latent = model.get_first_stage_encoding(model.encode_first_stage(init_rand))  # move to latent space
                            ...
                            init_image_show = cv2.cvtColor(init_image_show, cv2.COLOR_BGR2RGB)
                            ...
                            init_latent_image = cv2.cvtColor(init_latent_image, cv2.COLOR_BGR2RGB)
                            init_latent_image = cv2.resize(init_latent_image, init_image_show.shape[:2])
                            ...
                            for n in trange(opt.n_iter, desc="Sampling"):
                                ...
                                for prompts in tqdm(data, desc="data"):
                                    ...
                                    ###################
                                    z_enc_low = sampler.stochastic_encode(init_latent, torch.tensor([1]*batch_size).to(device))

                                    samples_show = latent2img(z_enc)
                                    # cv2.imwrite(os.path.join(sample_path, f"{image_name}_{base_count:04}_z_enc.png"), samples_show)
                                    samples_show = latent2img(z_enc_low)
                                    # cv2.imwrite(os.path.join(sample_path, f"{image_name}_{base_count:04}_z_enc_low.png"), samples_show)
                                  
                                    
                                    # mask_tensor = cv2.resize(mask_org, z_enc_low.shape[2:])[:,:,0].astype(bool)
                                    # mask_tensor = torch.from_numpy(mask_tensor).to(device)
                                    # mask_tensor = mask_tensor.expand(4,64,64).unsqueeze(0)

                                    # tensor_scaler_1 = -(mask_tensor*1) + 1
                                    # tensor_scaler_2 = mask_tensor*1

                                    # z_fusion = z_enc*tensor_scaler_1 + z_enc_low*tensor_scaler_2
                                    # # z_fusion = torch.mean(z_enc+z_enc_low, dim=0).unsqueeze(0)
                                    # # z_enc = latent_fusion(z_enc, z_enc_low, mask_org)
                                    # # z_enc = z_fusion

                                    # samples_show = latent2img(z_fusion)
                                    # cv2.imwrite(os.path.join(sample_path, f"{image_name}_{base_count:04}_z_fusion.png"), samples_show)
                                    # # cv2.imwrite(os.path.join(sample_path, f"{image_name}_{base_count:04}_latent_final.png"), samples_show)
                                    # surf = plot3d(samples_show)

                                    # # z_enc = sampler.stochastic_encode(init_latent, torch.tensor([t_enc]*batch_size).to(device), noise=z_enc_low)
                                    ###################
                                    ...
                                    for k in range(len(intermediates)):
                                        ...
                                        samples_show = cv2.cvtColor(samples_show, cv2.COLOR_BGR2RGB)
                                        samples_show = cv2.resize(samples_show, init_image_show.shape[:2])
                                        ...
                                        x_samples_show = cv2.cvtColor(x_samples_show, cv2.COLOR_BGR2RGB)
                                    ...
                                    if not opt.skip_save:
                                        for x_sample in x_samples:
                                            ...
                                            image.save(os.path.join(sample_path, f"{image_name}_{base_count:04}.png"))
                                            cv2.imwrite(os.path.join(sample_path, f"{image_name}_{base_count:04}_mask.jpg"), mask)
                                            base_count += 1
