...
def load_img(image, path, mask, noise_std):
    ...
    image = 1*image.astype(int) + 0*mask.astype(int)
                ...
                with model.ema_scope():
                    ...
                    while i < num_sample:
                        try:
                            ...
                            # opt.init_img = "./test_img/image_skeleton_dia.png"
                            # prompt_1 = "photo realistic, high resolution, wildfire in forest, smoke"
                            # prompt_1 = "flaw on cabinets, crack on windows"
                            # prompt_1 = "flaw on cabinets, crack on windows, HD, 4k, high resolution, realistic"
                            # prompt_1 = "flaw, crack, uneven paint on the door, HD, 4k, high resolution, realistic"
                            # prompt_1 = "realistic door, exhibiting a unique flaw such as cracks, warping, scratches, or discoloration, attention to detail and accuracy in architectural elements. The composition should enable close examination of the flaws, with lighting that accentuates the imperfections, flaw detection, emphasis on texture and material integrity"
                            # prompt_1 = "Ultra-high-definition, close-up image of a pronounced flaw on a furniture, such as a deep crack, significant warping, extensive scratches, or severe discoloration. The camera is positioned to capture the intricacies and texture of the damage. Lighting should be carefully arranged to highlight the depth and severity of the flaw, emphasizing its impact on the door's material integrity. The aim is to provide a clear and detailed examination of the flaw, suitable for educational or professional analysis in flaw detection. This visualization should focus solely on the flaw"
                            # prompt_1 = "Ultra-high-definition, image of a pronounced flaw on a wall, such as a deep crack, significant warping, extensive scratches, or severe discoloration. Emphasizing its impact on the material integrity. "
                            prompt_1 = "Ultra-high-definition, image of a pronounced flaw on a wall, such as uneven paint, dirt spot, deep crack, significant warping, extensive scratches, or severe discoloration. Emphasizing its impact on the material integrity. The camera is straight to the wall, wide angle."
                            ...
                            # for strength in np.arange(0.1, 1.1, 0.1):
                            # for noise_std in [0, 0.01,0.05,0.1,0.2,0.5]:
                            # for noise_std in [0]:

                            opt.scale = 5# random.randint(5, 15)
                            opt.strength = 0.99# random.uniform(0.6, 0.7)
                            # opt.scale = int(10/opt.strength)
                            opt.ddim_steps = int(11/(opt.strength))
                            # if opt.ddim_steps>=100: opt.ddim_steps=100
                            noise_std = 0.01 # random.uniform(0.1, 0.2)
                                ...
                                for prompts in tqdm(data, desc="data"):
                                    ...
                                    if not opt.skip_save:
                                        for x_sample in x_samples:
                                            ...
                                            image_save_name = f"{image_name}_{i}_{noise_std}_.png"
                                            mask_save_name = f"{image_name}_mask.png"
