...
def load_img(image_name, path, noise_std):
    ...
    # To tensor
    image = np.array(noisy_image).astype(np.float32) / 255.0
            ...
            with model.ema_scope():
                ...
                while i < num_sample:
                    try:
                        ...
                        opt.scale = 30 #random.randint(5, 10)
                        opt.strength = 0.5 #random.uniform(0.1, 0.45)
                        # opt.scale = int(10/opt.strength)
                        noise_std = 0 #random.uniform(0.01, 0.5)
