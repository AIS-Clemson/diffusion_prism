...
def load_img(image_name, path, noise_std):
    ...
    # Add noise
    mask = np.array(mask).astype(np.float32) / 255.0
    mask = mask + np.random.normal(0, noise_std, size=(h,w))
    # plt.imshow(image)
...
if __name__ == "__main__":
    ...
    if batch_generation:
                ...
                with model.ema_scope():
                    ...
                    while i < 1:
                        try:
                            ...
                            opt.prompt = 'wildfire'
                            # opt.prompt = "photo realistic, wildfire in the forest, flame and fire in the smoke, snowing, high resolution"
                            # opt.prompt = "photo realistic, cars and people in the city, city view, traffic lights, high resolution"
                            # opt.prompt = "wildfire in the forest"
                            ...
                            noise_std = random.uniform(0.1, 0.2)
                            ...
                            # opt.init_img = "./train/dendrite_sample/b1.png"
                            ...
                            plt.figure()
                            plt.imshow(Image.open(opt.init_img))
                            plt.imshow(mask)
        
                            for i in range(4):
                                latent = init_latent[0][i].detach().cpu().numpy()
                                plt.figure()
                                plt.imshow(latent)
                            ...
                            for n in trange(opt.n_iter, desc="Sampling"):
                                ...
                                for prompts in tqdm(data, desc="data"):
                                    ...
                                    for i in range(4):
                                        mask_latent = torch.tensor(cv2.resize(mask, (64,64)), dtype=float).to(device) /200
                                        z_enc[0][i] += mask_latent

                                        # z = z_enc[0][i].detach().cpu().numpy()
                                        plt.figure()
                                        plt.imshow(z_enc[0][i].cpu())
                                    ...
                                    # samples, cc = sampler.decode(init_latent, c, t_enc, unconditional_guidance_scale=opt.scale,
                                    #                           unconditional_conditioning=uc,)
            
            
                                    for i in range(4):
                                        sample = samples[0][i].detach().cpu().numpy()
                                        plt.figure()
                                        plt.imshow(sample)
                                    ...
                                    image_RGB = x_samples[0].detach().cpu().permute(1,2,0).numpy()
                                    plt.figure()
        ...
        #     # Average across heads (optional)
        #     average_attention_to_wildfire = attention_to_wildfire_reshaped[i,:,:]  # shape becomes 64x64
        ...
        # for i in range(77):
        #     attention_to_wildfire = attention_maps[:, :, i]  # Extracting the vectors related to "wildfire", shape 16x4096
        ...
        # for i in range(channel):
        #     attention_map_i = attention_maps[i]
        #     # attention_map_i = norm_image(attention_map_i)
    
        #     heat_map = attention_map_i.reshape((64,64,77))
        #     heat_map_i = heat_map[:,:,0]
        #     heat_map_i = norm_image(heat_map_i)
        #     # heat_map_i = heat_map.sum(axis=2)  # shape becomes 64x64

        #     heat_map_i = cv2.resize(heat_map_i, (512,512))
            
        #     plt.figure(dpi=100)
        #     plt.title(f'ID = {i}')
        #     plt.imshow(image_RGB)
        #     plt.imshow(heat_map_i, alpha=0.5, interpolation='nearest', cmap='jet')
        ...
        #     for j in range(320):
        #         mean_1 += attention_maps[i][0].transpose(1,2,0)[:,:,j]
        #         mean_2 += attention_maps[i][1].transpose(1,2,0)[:,:,j]
        # plt.figure()
        # plt.imshow(mean_1)
        # plt.figure()
        # plt.imshow(mean_2)
