...
def load_img(image, path, mask, noise_std):
    ...
    mask_fire = copy.deepcopy(cv2.cvtColor(mask, cv2.COLOR_RGB2BGR))
    ...
    # image[image>255] = 255
    mask_comb = copy.deepcopy(cv2.cvtColor(image.astype(np.uint8), cv2.COLOR_RGB2BGR))
    ...
    return (2.*image - 1.), mask_comb, mask_fire, [R,G,B]
...
if __name__ == "__main__":
    ...
    if batch_generation:
        ...
        opt.outdir = f'./test_data/exp/perli_plot/'
                ...
                with model.ema_scope():
                    ...
                    while i < num_sample:
                        try:
                            ...
                            prompt_1 = "wildfire in forest, flame and smoke, drone view, photo realistic, high resolution, 4k, HD"
                            ...
                            mask.save(os.path.join(outpath, f"{image_name}_{i}_mask.jpg"))
                            ...
                            init_image, mask_comb, mask_fire, [R,G,B] = load_img(image, opt.init_img, mask, noise_std)
                            ...
                            cv2.imwrite(os.path.join(outpath, f"{image_name}_{i}_mask_fire.jpg"), mask_fire)
                            cv2.imwrite(os.path.join(outpath, f"{image_name}_{i}_mask_comb.jpg"), mask_comb)

                            ###################
                            init_image_show = tensor2img(init_image)
                            init_image_show = cv2.cvtColor(init_image_show, cv2.COLOR_BGR2RGB)
                            cv2.imwrite(os.path.join(outpath, f"{image_name}_{i}_init_tensor.png"), init_image_show)
                            ...
                            cv2.imwrite(os.path.join(outpath, f"{image_name}_{i}_init_image.png"), init_latent_image)
                            ...
                            for n in trange(opt.n_iter, desc="Sampling"):
                                ...
                                for prompts in tqdm(data, desc="data"):
                                    ...
                                    for k in range(len(intermediates)):
                                        ...
                                        cv2.imwrite(os.path.join(outpath, f"{image_name}_{i}_latent_{k}.png"), samples_show)
                                        ...
                                        cv2.imwrite(os.path.join(outpath, f"{image_name}_{i}_image_{k}.png"), x_samples_show)
                                    ...
                                    if not opt.skip_save:
                                        ...
                                        for x_sample in x_samples:
                                            ...
                                            image.save(os.path.join(outpath, image_save_name))
                                            # cv2.imwrite(os.path.join(outpath, f"{image_name}_{i}_init_latent_image.png"), init_latent_image)
