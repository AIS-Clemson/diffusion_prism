...
def load_img(image, path, mask, noise_std):
    ...
    mask[:, :, 0] = mask[:, :, 0] * np.random.uniform(0, 1)
    mask[:, :, 1] = mask[:, :, 1] * np.random.uniform(0, 1)
    mask[:, :, 2] = mask[:, :, 2] * np.random.uniform(0, 1)
                    ...
                    while i < num_sample:
                        try:
                            ...
                            for noise_std in [0]:
                                ...
                                # image_RGB = init_latent[0].detach().cpu().permute(1,2,0).numpy()
                                # image_RGB = z_enc[0].detach().cpu().permute(1,2,0).numpy()
                                # image_RGB = samples[0].detach().cpu().permute(1,2,0).numpy()
                                ...
                                # image = np.stack((x1,x2,x3))
                                # image = np.transpose(image, (1,2,0))
                                # image = (image - np.min(image))/(np.max(image)-np.min(image)) *255
                                # image = cv2.resize(image, (512,512))

                                
                                # cv2.imwrite('latent.png', image)
                                    ...
                                    for prompts in tqdm(data, desc="data"):
                                        ...
                                        if not opt.skip_save:
                                            for x_sample in x_samples:
                                                ...
                                                image.save(
                                                    os.path.join(sample_path, f"{image_name}_{i}_{noise_std}_.png"))
