...
def load_img(image_name, path, noise_std):
    ...
    for i in range(w):
        for j in range(h):
            if mask[i,j] != 0:
                image[i,j,0] = 255*np.random.uniform(0.9, 1) 
                image[i,j,1] = 255*np.random.uniform(0.5, 0.9)
                image[i,j,2] = 255*np.random.uniform(0, 0.5)
    ...
    # mask[:, :, 1] = mask[:, :, 1] * np.random.uniform(0.39, 0.59)
    ...
    # image[mask>100] = 0.5*image[mask>100] + 0.5*mask[mask>100]
    # image[image>255] = 255
    # image = image.astype(np.uint8)
    # plt.imshow(image)
                                ...
                                for prompts in tqdm(data, desc="data"):
                                    ...
                                    if not opt.skip_save:
                                        for x_sample in x_samples:
                                            ...
                                            cv2.imwrite(os.path.join(sample_path, f"{image_name}_{base_count:04}_mask.png"), mask)
                                            base_count += 1
                                    all_samples.append(x_samples)
