...
class CustomDataset(Dataset):
    def __init__(self, image_paths, mask_paths, trimap_paths):
        self.image_paths = image_paths
        self.mask_paths = mask_paths
        self.trimap_paths = trimap_paths
    ...
    def __getitem__(self, idx):
        ...
        trimap_path = os.path.join(self.trimap_paths, image_name)
        ...
        trimap = Image.open(trimap_path).convert('L')  # Convert mask to grayscale (single channel)
...
custom_dataset = CustomDataset(image_paths, mask_paths, trimap_paths)
train_loader = DataLoader(custom_dataset, batch_size=8, shuffle=True)

# Test the data loader
for batch in train_loader:
    images, masks, trimaps = batch
    ...
    print(trimaps.size())   # torch.Size([32, 1, 256, 256]) (batch_size=32, single-channel masks, 256x256 size)
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        ...
        combined_image = display_result(images, trimaps, outputs)
